...
source_dir_02 = "./../../12_run_inception/05_cnn_inc/data/03_mid_train_extr/" 
...
des_dir = os.path.abspath(os.path.join(stored_dir, "04_post_train_data_concat")) 
...
#for nFile in range(0,4):
    ...
    if nClass != 100:
        ...
        # For drawing
        if (nFile == 0):
            mul_seq_x  =  np.concatenate((file_str, file_str_02), axis = 1)
            #print(np.shape(mul_seq_x), np.shape(file_str_02), np.shape(file_str))
            #exit()
            mul_seq_y  =  seq_y
        else:    
            concat_data=  np.concatenate((file_str, file_str_02), axis = 1)
            mul_seq_x  =  np.concatenate((mul_seq_x, concat_data), axis=0)
