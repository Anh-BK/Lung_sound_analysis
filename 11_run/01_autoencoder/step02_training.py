...
# 1.1/ Directory TODO
tf.flags.DEFINE_string("TRAIN_DIR",  "./../../02_data_enc_dec/04_data_10s_icb/data/data_train/", "Directory of feature")
tf.flags.DEFINE_string("VALID_DIR",  "./../../02_data_enc_dec/04_data_10s_icb/data/data_test/", "Directory of feature")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        with tf.control_dependencies(update_ops):
            ...
            grads_and_vars = optimizer.compute_gradients(model.losses)
        ...
        ### loss_summary     = tf.summary.scalar("loss", model.losses)
        ...
        def train_process(x_train_batch, y_train_batch):
            ...
            feed_dict= {model.i_image:     x_train_batch,
                        model.i_exp_class: y_train_batch,
                        model.mode:        True
                       }
            [ _, step, loss_cmp, losses] = sess.run([train_op, global_step, model.loss_cmp, model.losses], feed_dict)

            return loss_cmp, losses

        def test_one_batch(x_test_batch, y_test_batch):

            feed_dict= {model.i_image:     x_test_batch,
                        model.i_exp_class: y_test_batch,
                        model.mode:        False
                       }
            [loss_cmp, losses] = sess.run([model.loss_cmp, model.losses], feed_dict)
        ...
        def test_process(x_test_batch):
            ...
            [step, loss_cmp] = sess.run([global_step, model.loss_cmp], feed_dict)
                        ...
                        if (current_step % FLAGS.CHECKPOINT_EVERY == 0):  
                            ...
                            if(is_testing):
                                ...
                                for nTestBatch in range(int(test_mul_batch_num/FLAGS.BATCH_SIZE)):
                                    ...
                                    loss_cmp = test_one_batch(x_test_batch, y_test_batch)
                                    acc_mul_batch_0 = acc_mul_batch_0 + loss_cmp
                            ...
                            if(nEpoch == FLAGS.NUM_EPOCHS):         
                                ...
                                is_break = 1
                            elif(old_ave_acc <= 0.000001):
                                print("Break at 0.000001")
                                is_break = 1
